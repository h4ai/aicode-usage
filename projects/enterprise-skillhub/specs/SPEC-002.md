...
- **响应体**: 更新后的 Skill 对象
- **错误码**: `403 Forbidden` (非作者或管理员)
- **权限要求**: Skill 的作者 或 ADMIN
...
2. **可见性规则**:
   ...
   - `DEPARTMENT`: 仅 `allowedDepts` 列表中包含的部门的用户、Skill 作者、ADMIN 可见。
   - `PRIVATE`: 仅作者、ADMIN 可见。用于开发中或仅自用的 Skill。
...
- **页面**: 
  ...
  - 我的发布页 (My Skills)
- **组件**:
  - `SkillCard`: 展示名称、分类、作者、Star 数、徽章。
  - `CategoryFilter` / `TagCloud`: 左侧过滤边栏。
  - `VisibilitySelector`: 创建/编辑时的下拉或单选组件，当选 Department 时级联显示部门多选框。

## 6. 安全要求（认证、权限矩阵、数据脱敏、审计日志）
- 必须进行越权检测（IDOR防范）：在执行 UPDATE 和 DELETE 时，必须校验当前登录用户的 ID 是否与 `skill.authorId` 匹配，或者当前用户拥有 ADMIN 角色。

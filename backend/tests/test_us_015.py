...
_MOCK_LEADERBOARD = [
    {"rank": 1, "user_id": "user1", "display_name": "张三", "enterprise": "Engineering",
     "quota_level": "L2", "monthly_token": 8000, "monthly_requests": 400, "quota_usage_pct": 80.0},
    {"rank": 2, "user_id": "user2", "display_name": "李四", "enterprise": "未知",
     "quota_level": "L1", "monthly_token": 4000, "monthly_requests": 150, "quota_usage_pct": 40.0},
]
...
def test_ac2_response_has_required_fields(client, admin_token, admin_config_patch):
    ...
    with patch("app.routers.admin.get_leaderboard", return_value=_MOCK_LEADERBOARD):
        resp = client.get("/api/admin/leaderboard",
            headers={"Authorization": f"Bearer {admin_token}"})
    item = resp.json()[0]
    for field in ("rank", "display_name", "enterprise", "quota_level", "monthly_token", "monthly_requests", "quota_usage_pct"):
